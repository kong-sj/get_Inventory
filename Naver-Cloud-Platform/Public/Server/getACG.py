...
def getAccessControlGroupList(accesskey,secret_key,timestamp):
    ...
    host = "https://ncloud.apigw.ntruss.com"#민간존
    uri = "/vserver/v2/getAccessControlGroupList?regionCode=KR&responseFormatType=json" #? 뒤는 옵션인데 한글로 되어있고 json으로 된 것을
    url = host + uri
...
def getAccessControlGroupRuleList(accesskey,secret_key,timestamp,ACG_No):
    method = "GET"# Rest API 요청 형태, 조회할때 씀
    host = "https://ncloud.apigw.ntruss.com"#민간존
    uri = "/vserver/v2/getAccessControlGroupRuleList?regionCode=KR&responseFormatType=json&accessControlGroupNo="+ACG_No #? 뒤는 옵션인데 한글로 되어있고 json으로 된 것을
    url = host + uri
...
def getAccessControlGroupDetail(accesskey,secret_key,timestamp,ACG_No):
    method = "GET"# Rest API 요청 형태, 조회할때 씀
    host = "https://ncloud.apigw.ntruss.com"#민간존
    uri = "/vserver/v2/getAccessControlGroupDetail?regionCode=KR&responseFormatType=json&accessControlGroupNo="+ACG_No #? 뒤는 옵션인데 한글로 되어있고 json으로 된 것을
    url = host + uri
...
def getNetworkInterfaceList(accesskey,secret_key,timestamp):
    method = "GET"# Rest API 요청 형태, 조회할때 씀
    host = "https://ncloud.apigw.ntruss.com"#민간존
    uri = "/vserver/v2/getNetworkInterfaceList?regionCode=KR&responseFormatType=json"#? 뒤는 옵션인데 한글로 되어있고 json으로 된 것을
    url = host + uri
...
def getServerInstanceDetail(accesskey,secret_key,timestamp,Instance_No):
    method = "GET"# Rest API 요청 형태, 조회할때 씀
    host = "https://ncloud.apigw.ntruss.com"#민간존
    uri = "/vserver/v2/getServerInstanceDetail?regionCode=KR&responseFormatType=json&serverInstanceNo="+Instance_No #? 뒤는 옵션인데 한글로 되어있고 json으로 된 것을
    url = host + uri
...
def getVpcDetail(accesskey,secret_key,timestamp,Vpc_No):
    method = "GET"# Rest API 요청 형태, 조회할때 씀
    host = "https://ncloud.apigw.ntruss.com"#민간존
    uri = "/vpc/v2/getVpcDetail?regionCode=KR&responseFormatType=json&vpcNo="+Vpc_No#? 뒤는 옵션인데 한글로 되어있고 json으로 된 것을
    url = host + uri
...
accesskey = "C260ABBD08AA01AE1A14"
secret_key = "455D4FE08CE7961910254734E1BD570B616346D9"
